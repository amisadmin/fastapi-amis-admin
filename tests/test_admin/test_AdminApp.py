...
async def test_register_admin(site: AdminSite):
    ...
    assert app.db
    assert app.engine
    ins = app.get_admin_or_create(CategoryAdmin)
    assert ins in app
    assert app.get_model_admin(Category.__tablename__)

    site.register_router()
    assert site.get_model_admin(Category.__tablename__)
...
async def test_get_model_admin(site: AdminSite):
    ...
    site.register_router()
    assert site.get_model_admin(Category.__tablename__) is None
