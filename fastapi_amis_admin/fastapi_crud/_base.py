...
class RouterMixin:
    ...
    def get_router(self) -> APIRouter:
        ...
        if self.router_permission_depend is not None:
            self.router.dependencies.append(Depends(self.router_permission_depend))
        return self.router
...
class BaseCrud(RouterMixin):
    ...
    def register_crud(self,
        ...
        self.schema_filter = schema_filter or self.schema_filter or schema_create_by_schema(
            self.schema_list, 'Filter', set_none=True)
        ...
        self.schema_update = schema_update or self.schema_update or \
                             schema_create_by_schema(self.schema_model, 'Update', exclude={self.pk}, set_none=True)
    ...
    async def has_list_permission(self, request: Request,
                                  paginator: Paginator,
                                  filter: "self.schema_filter" = None  # type: ignore
                                  , **kwargs) -> bool:
        return True
    ...
    def error_not_found_item(self, request: Request):
        raise HTTPException(status.HTTP_404_NOT_FOUND, "not found item")
